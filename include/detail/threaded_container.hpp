...
namespace detail {
template <typename T>
inline void push(T const& item, std::vector<T>& cont) {
  cont.push_back(item);
}
template <typename T>
...
template <typename T>
inline void push(T const& item, std::deque<T>& cont) {
  cont.push_back(item);
}
template <typename T>
...
template <typename T>
inline void push(T const& item, std::priority_queue<T>& cont) {
  cont.push(item);
}
template <typename T>
...
class threaded_container {
 ...
 public:
  ...
  void push(const value_type& item, std::lock_guard<std::mutex>& lk) {
    detail::push(item, *this);
